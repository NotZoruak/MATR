...
public class MaaProcessor
{
    ...
    private static readonly int[][] FormationEquipTeamClickCoords =
    [
        [348, 387, 59, 29], [526, 388, 59, 29], [701, 390, 59, 29], [875, 387, 59, 29], [351, 458, 60, 30],
    ];
...
// 防止 interface 加载失败时 Toast 重复显示
    ...
    private void ApplyFormationPresetOverride(ref MaaToken taskModels, DragItemViewModel task, int? formationPresetId)
    {
        ...
        var overrides = new Dictionary<string, JToken>
        {
            ...
            ["FC_ClickEquipCond4"] = new JObject
            {
                ["action"] = new JObject
                {
                    ["param"] = new JObject
                    {
                        ["target"] = new JArray(FormationEquipTeamClickCoords[team - 1]),
                    },
                },
            },
