...
全局页的改动必须落盘到实例配置的 `GlobalOptionItems`，重启后保持用户选择。保存逻辑不得再依赖任务列表中的 `__GlobalOption__` 合成项：该项已按本定制从任务列表移除，取不到时应直接保存内存中的 `GlobalSelectOptions`，否则开关状态只在本次运行内生效。
...
主窗口默认尺寸为 900×675（宽×高），由 `RootView.axaml` 的 `Width` 与 `Height` 指定，`MinWidth` 700 与 `MinHeight` 360 保持不变。该默认值只在没有保存过窗口大小时生效（用户调整过的窗口大小由 `MainWindowWidth`/`MainWindowHeight` 恢复），升级时不得恢复上游默认尺寸。
...
已同步 MFAAvalonia `19a8416`：任务失败时同样执行“任务结束后操作”。例如“结束进程”关闭模拟器导致连接断开、队列失败后，用户配置的关闭 MATR 仍须执行；用户手动停止的分支保持不触发结束后操作。
...
外部触发与应用内计时器必须共用同一份触发标记：系统计划任务转发（命名管道）与命令行 `--autostart` 的冷启动都要调用 `TimerModel.MarkScheduledStartHandled`，把命中的定时器在 `LastTriggered` 上标记为本分钟已触发。判定由 `Helper/ValueType/TimerExternalStartMatch.cs` 负责：只覆盖启用中、动作为「开始任务」、设定时间与本次启动同分钟、重复规则当天命中且目标实例一致的定时器；跟随当前实例的定时器没有固定目标，外部启动已经把该实例切为当前实例，因此按覆盖处理。不得只在 `TriggerTimer` 内部记录触发时间，也不得让外部启动跳过回写：2026-09-13 15:30 实测缺少回写时，系统计划任务在 15:30:00 启动任务后，应用内 1 分钟计时器在 15:30:51 又把同一个定时器触发一次，配合强制定时启动把刚启动的任务停掉重启，唤醒本丸因此在同一分钟内重启了两次游戏。
...
### `resource.sword-drop-notification-gui-log`
